...
| 级别 | 条件 | 产物 |
|------|------|------|
| **C类 - 跳过原型** | B类板块中的单点功能或功能项 · 纯后台需求、简单字段调整、无前端页面、交互无新增（例：促销规则改动、商品价格变化） | 不生成原型，直接进入技术方案 |
| **B类 - 原型骨架** | A类产品中的一个业务板块/子域 · 页面结构清晰，部分细节或状态待确认（例：促销模块、返利模块、商城） | 含主要布局和关键组件的 HTML，未确认部分用占位符 |
| **A类 - 完整原型** | 端到端独立产品 · 完整业务闭环 · 核心项已确认，页面清单/字段/状态/异常态完备（例：OTC、订货通、服务通） | 包含所有页面、状态、交互的完整 HTML |
...
当原型设计过程中产生了 PRD 中未覆盖的新内容（如新增页面、新增模块、交互细节补充、字段调整等），**必须遵循以下规则**：

1. **先询问，再反写**：在反向更新 PRD 之前，必须先用明确的问题询问用户是否需要更新 PRD。不得在用户未确认的情况下自动修改 PRD 文件。
2. **列出变更清单**：询问时应列出待反写的具体内容（如新增的页面/模块名称、补充的字段/交互描述等），让用户逐项确认或拒绝。
3. **用户选择权**：用户可以选择全部接受、部分接受或全部拒绝反向更新。尊重用户的选择，仅更新被确认的项。
4. **反写后标记**：反写完成后，在 PRD 文档中添加变更记录（时间、变更内容、变更来源）。

推荐询问格式：

```markdown
原型设计中产生了以下 PRD 未覆盖的新内容，是否需要反向更新 PRD？

1. 新增「运营建议模块」— 详情页按渠道分级动态生成差异化运营建议
2. 新增「风险预警列表页」— 独立预警列表页，含统计卡片、AI分析摘要、筛选联动

你可以回复 1/2 选择要反写的项，或回复"全部"/"不需要"。
```

> **注意**：原型中已存在但 PRD 中已有对应描述的内容（如页面布局、图表类型等）不需要反写。仅反写 PRD 中完全缺失的新增内容。
